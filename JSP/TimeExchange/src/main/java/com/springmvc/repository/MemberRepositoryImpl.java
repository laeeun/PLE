...
public class MemberRepositoryImpl implements MemberRepository {
	...
	public void save(Member member) {
		...
		template.update(sql, member.getMember_id(), // member_id
	...
	public void update(Member member) {
		...
		template.update(sql, member.getUserName(), member.getName(), member.getEmail(), member.getPhone(),
				member.getBirthDate(), member.getGender(), member.getAddr(), member.getAddrDetail(), member.isExpert(),
				member.isEmailVerified(), member.getEmailToken(), member.getTokenCreatedAt(), member.getProfileImage(),
				member.getMember_id());
	}
	...
	public int restore(String member_id) {
	    String sql = "UPDATE member SET status = 'ACTIVE', login_fail_count = 0, last_fail_time = NULL WHERE member_id = ?";
	    return template.update(sql, member_id);
	}
