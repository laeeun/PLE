...
.btn-submit:hover { transform: translateY(-2px); box-shadow: 0 12px 30px rgba(255,107,53,.3); }
.btn-submit:focus, .btn-submit:focus-visible {
  outline: none;
...
.category-grid .form-check-label:hover {
  border-color: var(--accent);
  box-shadow: 0 6px 18px rgba(0,0,0,.06);
}

/* 체크되면 타일 하이라이트 */
.category-grid .form-check-input:checked + .form-check-label {
  border-color: var(--accent);
...
/* 키보드 포커스 */
.category-grid .form-check-input:focus + .form-check-label,
.category-grid .form-check-input:focus-visible + .form-check-label {
  outline: none;
  box-shadow: 0 0 0 3px rgba(255,107,53,.15);
}
...
#uploadFile::-webkit-file-upload-button {
  ...
  cursor: pointer;
  margin-right: 12px;
  box-shadow: 0 8px 20px rgba(255,107,53,.18);
  transition: transform .15s ease, box-shadow .2s ease, opacity .2s ease;
}
#uploadFile:hover::file-selector-button,
#uploadFile:hover::-webkit-file-upload-button {
  transform: translateY(-1px);
  box-shadow: 0 12px 28px rgba(255,107,53,.26);
}

@media (max-width: 768px) {
  .container { margin: 40px 20px; padding: 30px 24px; }
  h2 { font-size: 28px; }
}
</style>
</head>
