...
public interface MemberRepository {
    ...
    Member findById(String id);                    // ID로 회원 조회
    
    List<Member> findAll();                        // 전체 회원 조회
    
    void update(Member member);                    // 회원 정보 수정
    
    void delete(String id);                        // 회원 삭제

    // 확장 기능
    Member login(String id, String password);      // 로그인 처리
    ...
    List<Member> searchByKeyword(String keyword);  // 키워드로 검색
